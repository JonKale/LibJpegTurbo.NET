namespace LibJpegTurbo.Net
{
    ...
    public static class TurboJpegInterop
    {
        /// <summary>
        ...
        /// <paramref name="/>" />
        /// You should always use this function to allocate the JPEG destination buffer(s) for <see cref="compress" />
        /// and <see cref="transform" /> unless you are disabling automatic buffer (re)allocation (by setting
        /// <see cref="TurboJpegFlags.NoReallocation" />).
        /// </summary>
        /// <param name="size">The size.</param>
        /// <returns></returns>
        [DllImport("turbojpeg.dll", EntryPoint = "tjAlloc")]
        ...
        /// The reason for this is that the JPEG format uses 16-bit coefficients, and it is thus possible for a very
        ...
        /// height, and chroma subsampling.
        /// </returns>
        [DllImport("turbojpeg.dll", EntryPoint = "tjBufSize")]
        ...
        /// <param name="pad">
        /// The width of each line in each plane of the image is padded to the nearest multiple of
        ...
        /// <returns>
        /// The size of the buffer (in bytes) required to hold a YUV planar image with the given width,
        /// height and chroma subsampling.
        /// </returns>
        [DllImport("turbojpeg.dll", EntryPoint = "tjBufSizeYUV")]
        ...
        /// <param name="pitch">
        /// The bytes per line of the source image. Normally, this should be
        /// <paramref name="width" /> * <see cref="TurboJpegUtilities.GetPixelSize" />(<paramref name="pixelFormat" />) if
        /// the image is unpadded, or <see cref="TurboJpegUtilities.Pad" />(<paramref name="width" /> *
        ...
        /// A pointer to an image buffer that will receive the JPEG image. TurboJPEG
        /// has the ability to reallocate the JPEG buffer to accommodate the size of the JPEG image. Thus, you can
        /// choose to:
        /// 1. pre-allocate the JPEG buffer with an arbitrary size using <see cref="alloc" /> and let TurboJPEG grow the
        /// buffer as needed, or
        /// 2. set <paramref name="destinationBuffer" /> to NULL to tell TurboJPEG to allocate the buffer for you, or
        /// 3. pre-allocate the buffer to a "worst case" size determined by calling <see cref="bufSize" />. The buffer
        /// should never be re-allocated; to ensure that it is not use <see cref="TurboJpegFlags.NoReallocation" />
        /// and if the buffer is invalid or too small an error will be generated.
        ...
        /// <paramref name="destinationBuffer" /> upon return from this function, as it may have changed.
        /// </param>
        /// <param name="bufferSize">
        /// Size of the compressed image. If <paramref name="destinationBuffer" /> points to a
        /// pre-allocated buffer, then <paramref name="bufferSize" /> should be set to the size of the buffer. Upon
        /// return, <paramref name="bufferSize" /> will contain the size of the JPEG image (in bytes).
        /// </param>
        /// <param name="jpegSubsamp">The chroma subsampling to be used when generating the JPEG image.</param>
        ...
        [DllImport("turbojpeg.dll", EntryPoint = "tjCompress2")]
        ...
        /// <param name="destinationBuffer">
        /// An image buffer that will receive the decompressed image. This buffer
        ...
        /// The desired width (in pixels) of the destination image. If this is different
        ...
        /// </param>
        /// <param name="pitch">
        /// The number of bytes per line of the destination image. Normally, this is
        ...
        /// <paramref name="pixelFormat" />).
        /// </param>
        /// <param name="desiredHeight">
        /// The desired height (in pixels) of the destination image. If this is different
        ...
        /// set to 0, then only the width will be considered when determining the scaled image size.
        /// </param>
        /// <param name="pixelFormat">The pixel format of the destination image.</param>
        /// <param name="flags">The flags.</param>
        /// <returns>0 on success, or -1 if an error occurred.</returns>
        [DllImport("turbojpeg.dll", EntryPoint = "tjDecompress2")]
        ...
        /// <returns>0 on success, or -1 if an error occurred.</returns>
        [DllImport("turbojpeg.dll", EntryPoint = "tjDecompressHeader2")]
        ...
        /// </summary>
        /// <remarks>
        /// This function performs JPEG decompression but leaves out the color conversion step, so a planar YUV image
        ...
        /// community, the TurboJPEG API uses "YUV" to refer to an image format consisting of Y, Cb, and Cr image
        /// planes.
        /// </remarks>
        /// <param name="handle">A handle to a TurboJPEG decompressor or transformer instance.</param>
        ...
        /// <param name="flags">The flags.</param>
        /// <returns>0 on success, or -1 if an error occurred.</returns>
        [DllImport("turbojpeg.dll", EntryPoint = "tjDecompressToYUV")]
        ...
        /// <returns>0 on success, or -1 if an error occurred.</returns>
        [DllImport("turbojpeg.dll", EntryPoint = "tjDestroy")]
        ...
        /// </summary>
        /// <remarks>
        /// This function uses the accelerated color conversion routines in TurboJPEG's underlying codec to produce a
        ...
        /// <see cref="Subsampling.Chroma420" />, which produces an image compatible with the I420 (AKA "YUV420P") format.
        /// NOTE: Technically, the JPEG format uses the YCbCr colorspace, but per the convention of the digital video
        ...
        /// <param name="pixelFormat">The pixel format of the source image.</param>
        /// <param name="destinationBuffer">
        /// An image buffer that will receive the YUV image. Use <see cref="bufSizeYUV" />
        ...
        /// <param name="flags">The flags.</param>
        /// <returns>0 on success, or -1 if an error occurred.</returns>
        [DllImport("turbojpeg.dll", EntryPoint = "tjEncodeYUV2")]
        ...
        /// or <see cref="transform" /> or that were manually allocated using <see cref="alloc" />.
        /// </summary>
        /// <param name="buffer">The address of the buffer to free.</param>
        [DllImport("turbojpeg.dll", EntryPoint = "tjFree")]
        ...
        [DllImport("turbojpeg.dll", EntryPoint = "tjGetErrorStr")]
        ...
        /// TurboJPEG supports.
        /// </returns>
        [DllImport("turbojpeg.dll", EntryPoint = "tjGetScalingFactors")]
        ...
        [DllImport("turbojpeg.dll", EntryPoint = "tjInitCompress")]
        ...
        /// </summary>
        /// <returns>A handle to the newly-created instance, or <see cref="IntPtr.Zero" /> if an error occurred.</returns>
        [DllImport("turbojpeg.dll", EntryPoint = "tjInitDecompress")]
        ...
        /// </summary>
        /// <returns>A handle to the newly-created instance, or <see cref="IntPtr.Zero" /> if an error occurred.</returns>
        [DllImport("turbojpeg.dll", EntryPoint = "tjInitTransform")]
        ...
        /// <param name="destinationBuffers">
        /// An array of <paramref name="count" /> image buffers.
        ...
        /// 1. pre-allocate the JPEG buffer with an arbitrary size using tjAlloc() and let TurboJPEG grow the buffer as
        /// needed, or
        /// 2. set <paramref name="destinationBuffers" />[i] to <see cref="IntPtr.Zero" /> to tell TurboJPEG to allocate
        /// the buffer for you, or
        /// 3. pre-allocate the buffers to a "worst case" size determined by calling <see cref="bufSize" />. The buffers
        /// should never be re-allocated; to ensure that they are not use <see cref="TurboJpegFlags.NoReallocation" />
        /// and if a buffer is invalid or too small an error will be generated.
        ...
        /// </param>
        /// <param name="sizes">
        /// An array of <paramref name="count" /> unsigned long that will receive the actual sizes
        ...
        /// return, <paramref name="sizes" />[i] will contain the size of the JPEG image (in bytes).
        /// </param>
        /// <param name="transforms">
        /// Aan array of <paramref name="count" /> <see cref="TurboJpegTransform" /> structures,
        /// each of which specifies the transform parameters and/or cropping region for the corresponding transformed
        /// output image.
        /// </param>
        /// <param name="flags">The flags.</param>
        /// <returns>0 on success, or -1 if an error occurred.</returns>
        [DllImport("turbojpeg.dll", EntryPoint = "tjTransform")]
