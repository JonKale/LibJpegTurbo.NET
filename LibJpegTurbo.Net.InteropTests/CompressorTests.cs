namespace LibJpegTurbo.Net.InteropTests
{
    ...
    /// <summary>
    /// Summary description for CompressorTests
    /// </summary>
    [TestClass]
        ...
        public void CompressImageNoPreallocatedBuffer()
        {
            ...
            for (var y = 0; y < 64; ++y)
            {
                for (var x = 0; x < 64; ++x)
                {
                    ...
                    image.SetPixel(x, y, Color.FromArgb(colour >> 16, (colour >> 8) & 0xff, colour & 0xff));
            ...
            Assert.IsTrue(jpegImage.Width == width);
            Assert.IsTrue(jpegImage.Height == height);
            Assert.IsTrue(jpegImage.PixelFormat == SdiPixelFormat.Format24bppRgb);
            Assert.IsTrue(jpegImage.RawFormat.Equals(ImageFormat.Jpeg));
